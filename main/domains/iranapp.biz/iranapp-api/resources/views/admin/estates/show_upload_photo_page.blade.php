        ...
        <div class="content">
            <div class="container">
                <div class="card-box">
                    ...
                    <a href="{{ route('showAllEstateAdsInAdminPanel') }}"><b>بازگشت به لیست آگهی ها</b></a>
                    <div class="row">
                        @foreach($photos as $photo)
                            <div class="col-xs-6 col-md-4" style="position:relative;">
                                <a href="{{ route('deleteEstateAdsPhoto' , $photo->id) }}"
                                   class="btn btn-danger btn-xs" style="position:absolute;top:25px;left:40px;">حذف
                                </a>
                                <img src="{{ url()->to('/ads_photo') }}/{{ $photo->file_name }}" alt="ads photo" class="img-responsive img-thumbnail" width="400"/>
                            </div>
                        @endforeach
                    </div>
                    @if($available_photo_to_upload > 0)
                        <div class="row">
                            ...
                            <div class="col-xs-12">
                                <form action="{{ route('uploadEstateAdsPhotoInAdminPanel' , $ad->id) }}" method="post" enctype="multipart/form-data">
                                    {{csrf_field()}}
